...
BOOL CALLBACK   TaskManDlgProc  (HWND, UINT, WPARAM, LPARAM);
...
DWORD WINAPI StartTaskMan(LPVOID hWndParent)
{
    ...
    static TCHAR szStubWndClass[] = TEXT("TaskManStubWndClass");
    static TCHAR szStubWndTitle[] = TEXT("TaskManStubWndTitle");
    //
    ...
    if (!GetClassInfo(hinstCabinet, szStubWndClass, &wndclass))
    {
        wndclass.style         = 0;
        ...
        wndclass.lpszClassName = szStubWndClass;
    ...
    if (hwndstub = CreateWindow(szStubWndClass,            // class
                                szStubWndTitle,            // title
                                WS_POPUP,                  // style
...
BOOL CALLBACK TaskManDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch(msg)
    {
...
HRESULT GetWindowList(HWND hwndTasklist, HWND ** paHwnds, UINT * pcHwnds)
{
    ...
    *pcHwnds = 0;
    ...
    if (0 == cSelected)
    {
        ...
        aItems = (INT *) HeapAlloc(g_hHeap, 0, cItems * sizeof(INT));
        if (NULL == aItems)
        {
