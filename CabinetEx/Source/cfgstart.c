...
void CreateLink(LPITEMIDLIST pidl, LPCTSTR lpszDest)
{
    ...
    if (SUCCEEDED(ICoCreateInstance(&CLSID_ShellLink, &IID_IShellLink, &psl))) {
        ...
        PathCombineUnimpl(szLinkName, lpszDest, PathFindFileNameUnimpl(szBuffer));
        if (PathIsLink(szBuffer)) {
            ...
            psl->lpVtbl->SetDescription(psl, PathFindFileNameUnimpl(szBuffer));
            ...
            != -1)
    {
        ...
        if (!(lpmii->fFlags & fWant)) {
            ...
            if (lpmii->fFlags & fAlreadyIn) {
                ...
                PathAppendUnimpl(szPath, PathFindFileNameUnimpl(szFile));
                if (!PathIsLink) {
