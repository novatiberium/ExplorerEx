...
extern void PathRemoveExtensionUnimpl(LPTSTR pszPath);
...
void GenerateGroupName(LPTSTR lpszPath, LPTSTR lpszName)
{
    ...
    PathAppendUnimpl(lpszPath, lpszName);
...
BOOL _SameLastGroup(LPCTSTR lpszGroup)
{
        ...
        if (lpszGroup && g_pszLastGroupName)
        {
                ...
                {
                        LPTSTR pszName1 = PathFindFileNameUnimpl(lpszGroup);
                        LPTSTR pszName2 = PathFindFileNameUnimpl(g_pszLastGroupName);
...
void GetGroupPath(LPCTSTR pszName, LPTSTR pszPath, DWORD dwFlags, INT iCommonGroup)
{
    ...
    else
    {
        ...
        if (bFindPersonalGroup)
        {
            ...
            if (PathFileExistsUnimpl(pszPath))
                return;
            bCommonGroup = TRUE;
        }
...
BOOL DDE_CreateGroup(LPTSTR lpszBuf, UINT FAR* lpwCmd, PDDECONV pddec)
{
    ...
    if (!_SameLastGroup(szGroup))
    {
        ...
        if (!PathFileExistsUnimpl(pddec->szGroup))
        {
...
BOOL DDE_ShowGroup(LPTSTR lpszBuf, UINT FAR* lpwCmd, PDDECONV pddec)
{
    ...
    if (!PathFileExistsUnimpl(szGroup))
        return(FALSE);
...
BOOL DDE_DeleteGroup(LPTSTR lpszBuf, UINT FAR* lpwCmd, PDDECONV pddec)
{
    ...
    if (!PathFileExistsUnimpl(szGroupName))
        return(FALSE);
...
//---------------------------------------------------------------------------
BOOL PrivatePathStripToRootUnimpl(LPTSTR szRoot)
{
...
void Path_ChangeUNCToDrive(LPTSTR pszPath, TCHAR chDrive)
{
    ...
    PrivatePathStripToRootUnimpl(szPath);
    pszPath[0] = chDrive;
    pszPath[1] = TEXT(':');
    pszPath[2] = TEXT('\\');
...
BOOL DDE_AddItem(LPTSTR lpszBuf, UINT FAR* lpwCmd, PDDECONV pddec)
{
    ...
    if ((pddec->dwFlags & DDECONV_NO_UNC) && PathIsUNCUnimpl(szCL))
    {
        ...
        PrivatePathStripToRootUnimpl(szShare);
    ...
    if (nParams > 2)
    {
        ...
        if (*szTmp)
        {
            ...
            lpszArgs = PathGetArgsUnimpl(szTmp);
            if (*lpszArgs)
                *(lpszArgs-1) = TEXT('\0');
            // Save it.
            fIconPath = TRUE;
    ...
    if (!szTmp[0])
    {
        ...
        PathRemoveFileSpecUnimpl(szTmp);
    }

    // Don't use UNC paths.
    if (PathIsUNCUnimpl(szTmp))
    ...
    PathCombineUnimpl(szTmp, pddec->szGroup, szName);
    lstrcat(szTmp, c_szDotLnk);
    PathQualify(szTmp);

    // We need to handle link duplication problems on SFN drives.
    if (!IsLFNDrive(szTmp) && PathFileExistsUnimpl(szTmp))
...
BOOL DDE_DeleteItem(LPTSTR lpszBuf, UINT FAR* lpwCmd, PDDECONV pddec)
{
        ...
        PathCombineUnimpl(szPath, pddec->szGroup, &lpszBuf[*lpwCmd]);
...
HDDEDATA EnumGroups(HSZ hszItem)
{
        ...
        PathAppendUnimpl(szGroup, c_szStarDotStar);
...
void ConstructIconPath(LPTSTR pszIP, LPCTSTR pszCL, LPCTSTR pszWD)
{
    ...
    lstrcpy(sz, pszCL);
    PathRemoveArgsUnimpl(sz);
    PathUnquoteSpacesUnimpl(sz);
    ...
    if (SUCCEEDED(psf->lpVtbl->GetAttributesOf(psf, 1, &pidlLink, &dwAttribs)))
    {
        if (dwAttribs & SFGAO_LINK)
        {
            ...
            if (SUCCEEDED(psf->lpVtbl->GetDisplayNameOf(psf, pidlLink, SHGDN_NORMAL, &str)))
            {
                ...
                PathCombineUnimpl(sz, lpszGroupPath, szName);
