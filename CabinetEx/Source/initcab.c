        ...
        // but I'm not sure who put it there yet... double check.
            ...
            if (dwType == REG_SZ)
            {
                ...
                else
                {
                    ...
                    PathProcessCommandUnimpl(szCmdLine,
                                       szCmdLine, ARRAYSIZE(szCmdLine),
                                       PPCF_ADDARGUMENTS | PPCF_FORCEQUALIFY);

                    lpszArgs = PathGetArgsUnimpl(szCmdLine);
                    if (*lpszArgs)
                        *(lpszArgs-1) = TEXT('\0'); // Strip args

                    PathUnquoteSpacesUnimpl(szCmdLine);
      ...
      if (lstrlen(pStart) != 0)
        {
          ...
          pszFile = PathFindFileNameUnimpl(pStart);
          lstrcpy(szFile, pszFile);
          PathRemoveFileSpecUnimpl(pStart);
...
BOOL FPidlOnNonPersistentDrive(LPCITEMIDLIST pidl)
{
    ...
    if (PathIsUNCUnimpl(szPath) || !IsNetDrive(DRIVEID(szPath)))
    {
        fRet = FALSE;
        goto End;
    }
...
Error1:
    ...
    RegDeleteKeyA(g_hkeyExplorer, c_szSaveCmds);
}
        ...
        *puFlags = COF_NORMAL;
        ...
        if (*lpszCmdLine == TEXT('\0'))
        {
            *puFlags |= COF_CREATENEWWINDOW | COF_EXPLORE;
            ...
            while(*szField && !PathIsRootUnimpl(szField))
            {
                PathRemoveFileSpecUnimpl(szField);
            }
...
BOOL CreateFromDesktop(HINSTANCE hInst, LPCTSTR lpszCmdLine, int nCmdShow)
{
    ...
    if (hwndDesktop)
    {
        ...
        if (!(psCmdCopy->uFlags & COF_NEWROOT) && !psCmdCopy->pidl)
        {
            ...
            PathCombineUnimpl(szTemp, szTemp, psCmdCopy->pszPath);
...
void Cabinet_InitGlobalMetrics(WPARAM wParam, LPTSTR lpszSection)
{
    ...
    if (fForce || !lstrcmpi(lpszSection, c_szMetrics) ||
        ...
        ImageList_GetIconSizeUnimpl(g_himlSysLarge, &g_cxIcon, &g_cyIcon);
        ImageList_GetIconSizeUnimpl(g_himlSysSmall, &g_cxSmIcon, &g_cySmIcon);
...
BOOL FirstInstance()
{
    ...
    else
    {
        ...
        pszModuleName = PathFindFileNameUnimpl(szModuleName);
        ...
        do {
            ...
            pszT = PathGetArgsUnimpl(pszPortion);
            if (*pszT)
            ...
            pszT = PathFindFileNameUnimpl(pszPortion);
...
BOOL FindProgmanIni(LPTSTR pszPath, int cbPath)
{
    ...
    PathAppendUnimpl(pszPath, c_szProgmanIni);
...
void _HandleCmdLine(LPTSTR lpszCmdLine, UINT nCmdShow)
{
        ...
        {
                lpszArgs = PathGetArgsUnimpl(lpszCmdLine);
                if (*lpszArgs)
                        *(lpszArgs-1) = TEXT('\0');
