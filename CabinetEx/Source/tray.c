...
#include <shellapi.h>
#include "..\Headers\trayclok.h"
//#include <fsmenu.h>
//#include <shguidp.h>
//#include <trayp.h>
#include "..\Headers\traynot.h"
#include "..\Headers\help.h"      // help ids
#include "Shlwapi.h"
...
HWND g_hwndAutoHide[4] = { NULL, NULL, NULL, NULL };
    ...
    if (RegQueryValue(HKEY_CLASSES_ROOT, pszExt, sz, &cb) == ERROR_SUCCESS)
    {
        ...
        if (RegQueryValue(HKEY_CLASSES_ROOT, szExt, sz, &cb) == ERROR_SUCCESS)
        {
            ...
            if (PathIsRelativeUnimpl(pszCmdLine))
            {
                ...
                PathRemoveBlanksUnimpl(pszCmdLine);
...
// BUGBUG - BobDay - This code doesn't handle larger section
    ...
    for (pszLine = pszBuf; *pszLine; pszLine += lstrlen(pszLine)+1)
    {
        ...
        {
            if (lstrcmpi(PathFindFileNameUnimpl(szCmdLine), c_szDesignWDotExeSpace) == 0)
            {
        ...
        {
            if (lstrcmpi(PathFindFileNameUnimpl(szCmdLine), TEXT("WINORG.EXE ")) == 0)
            {
...
WORD _GetHotkeyFromPidls(LPITEMIDLIST pidlFolder, LPITEMIDLIST pidlItem)
{
    ...
    if (pidl)
    {
        ...
        if (SHGetPathFromIDList(pidl, szPath))
        {
            ...
            if (FAILED(_CLSIDFromExtension(PathFindExtensionUnimpl(szPath), &clsid)))
...
int HotkeyList_Restore(HWND hwnd)
{
    ...
    if (hkey)
    {
        ...
        RegDeleteKeyA(g_hkeyExplorer, c_szHotkeys);
    }
    return i;
}
        ...
        UINT idTitle, UINT idPrompt, DWORD dwFlags)
{
    ...
    if (szWorkingDir[0]) {
        idDrive = PathGetDriveNumberUnimpl(szWorkingDir);
        if ((idDrive != -1))
        {
            UINT dtype = DriveType(idDrive);
            if ( ( (dtype == DRIVE_REMOVABLE) || (dtype == DRIVE_CDROM) )
                 && !PathFileExistsUnimpl(szWorkingDir) )
            {
                SHGetSpecialFolderPath(hwnd, szWorkingDir, CSIDL_DESKTOPDIRECTORY,
                                   FALSE);
            }
...
DWORD WINAPI RunDlgThread(LPVOID data)
{
    ...
    if (hwnd) {
        ...
        if (PathAppendUnimpl(szDir,szPath) && PathIsDirectoryUnimpl(szDir))
        {
